...
export default function RadioGroupDefaultWrapper({ label, value = {}, onChange, options, onBlur, error }) {
    const getValue = (val) => {
      ...
      <Fragment>
          <RadioGroup value={getValue(value)} onChange={onChange} onBlur={onBlur}>
              ...
              <div className="space-y-2 mt-4">
                  {options.map((option) => (
                    <RadioGroup.Option key={option.id} value={option} className={({ active, checked }) => `${active
                                  ...
                                  <div className="flex items-center">
                                      <div className="text-sm">
                                          ...
                                          <RadioGroup.Description as="span"
                                                                  className={`inline ${checked ? 'text-orange-100' : 'text-gray-500'}`}>
